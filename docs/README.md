...
| 6. 用日志获取模型 | [离线辨识与整定流程](IDENTIFICATION.md) | 固定构形下辨识惯量/摩擦/重力，导出并验证候选 C 参数 |
...
可选 `GIMBAL_BUILD_IDENTIFICATION=ON` 增加两个电脑端测试入口，完整环境共11项；需Python/NumPy。新增辨识恢复与8个候选闭环案例均属合成验证，详见[辨识说明](IDENTIFICATION.md)。
