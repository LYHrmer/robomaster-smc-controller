...
Pitch 重力补偿由调用方提供：例如在“水平为零、正方向抬头”的特定机构中，可辨识 `tau_g(q)=m*g*l*cos(q+q0)`，将其送入 `tau_ff_nm`。实际符号、零位和传动比由机构决定。核心不根据惯性角度猜测重力臂。
...
3. 先关闭终端项、输出低通与额外复杂补偿；按机构允许范围设置力矩上限、斜率和参考速度/加速度上限。
...
6. 加入 Pitch 重力前馈；再决定是否启用速度低通、终端项或更平滑的上游参考。
