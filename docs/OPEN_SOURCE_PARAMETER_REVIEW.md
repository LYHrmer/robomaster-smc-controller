...
作者 [README](https://github.com/LamdaDay/YAW_Auto_Controller/blob/main/README.md) 明确指出，其模型依据命令力矩辨识，所得参数不是实际物理参数。因此 `J=0.039`、`Bv=0.30` 不能被标成经过 CAD 或独立力矩传感器确认的真实云台参数；尤其不能把执行链已包含在辨识结果里的影响再次叠加，然后称为忠实复现。若未来采用，应单列为“公开工程等效模型”的测试，而不是替代本轮正式 CAD/SDF 参数。
