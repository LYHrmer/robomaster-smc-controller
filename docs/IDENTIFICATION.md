...
**本页由 GM6020 和 DM4310 共用。** 尚未准备数据时，先读 [GM6020（6020）采集与辨识](GM6020_IDENTIFICATION.md) 或 [DM4310（4310）采集与辨识](DM4310_IDENTIFICATION.md)。电机反馈格式与标定步骤不同；转换成合格的关节侧数据后，使用同一套 CSV 和工具。
...
保留原工程的模式、INS、CAN与控制任务。建议在反馈快照完成后记录反馈，在最终执行器输出确定后记录命令与状态；由低优先级任务导出，避免在控制周期内阻塞打印。优先保存现有反馈原始量、原始报文、最终命令、时间戳和模式；请求力矩等字段按工程实际提供情况记录。电流原始值或驱动报告的力矩，只有经过比例、方向与传动标定后，才能作为关节侧 N·m 力矩估计。这里不假定旧固件已经输出标定力矩；完成标定与时间对齐后，再整理成下列精简 CSV 交给工具。

完整原始日志、实验配置和验证结果的保留规范见 [测试数据清单与模板](TEST_DATA.md)；下述精简 CSV 是离线工具输入格式。

电机侧前置步骤分别见 [GM6020：电流反馈与标定](GM6020_IDENTIFICATION.md) 和 [DM4310：MIT 反馈与标定](DM4310_IDENTIFICATION.md)。
