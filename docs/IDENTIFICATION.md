...
[文档导航](README.md) · [Pitch 整定](PITCH_TUNING.md) · [控制器设计](CONTROL_DESIGN.md)
...
完整环境下保留原9项测试，并增加辨识异常路径和导出参数闭环两个入口，共11项。该选项默认关闭，不增加 MCU 依赖。
...
后续在线RLS可先输出候选参数、激励指标和数据有效性，不直接修改闭环。发布参数需要独立的验证与回退路径。[RM2024-PowerModule](https://github.com/hkustenterprize/RM2024-PowerModule/blob/23613f8544a44060e91c4fc33b68c9668cfb5f3b/Utils/RLS.hpp)提供了递推估计的工程参考，但其对象是底盘功率损耗；其参数及更新器不能直接充当本库的动力学辨识与自动整定实现。
