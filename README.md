...
核心无 HAL、RTOS、堆内存和 C++ 依赖。Yaw、Pitch 可独立实例化，分别整定；电机更换通过适配层完成。

**当前状态：主机验证通过，包括两份开源模型的 576 个闭环组合；尚未完成 STM32/实机验证。** 模型参数有来源和离线复算过程，电机、延迟、噪声与控制器整定仍需匹配自己的云台。
