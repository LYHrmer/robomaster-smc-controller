...
**面向 STM32 的 C99 云台滑模控制器，提供 GM6020 / DM4310 接入与参数辨识。**

[![C99 controller tests](https://github.com/LYHrmer/gimbal-smc-controller/actions/workflows/c-tests.yml/badge.svg)](https://github.com/LYHrmer/gimbal-smc-controller/actions/workflows/c-tests.yml)
...
[快速开始](docs/QUICKSTART.md) · [GM6020 辨识](docs/GM6020_IDENTIFICATION.md) · [DM4310 辨识](docs/DM4310_IDENTIFICATION.md) · [STM32 接入](docs/STM32_PORT.md) · [全部文档](docs/README.md)
...
| **使用 GM6020（6020），准备采集与辨识** | [GM6020 专用指南](docs/GM6020_IDENTIFICATION.md)：电流反馈、模式核对、力矩标定 |
| **使用 DM4310（4310），准备采集与辨识** | [DM4310 专用指南](docs/DM4310_IDENTIFICATION.md)：MIT 反馈、量程与力矩标定 |
| 已有合格日志，开始拟合模型 | [通用离线辨识](docs/IDENTIFICATION.md) → [可选的 STM32 在线拟合](docs/ONLINE_IDENTIFICATION.md) |
...
```sh
git clone https://github.com/LYHrmer/gimbal-smc-controller.git
cd gimbal-smc-controller
...
| 电机 | 当前接入方式 | 协议与采集入口 |
|---|---|---|
| **GM6020（6020）** | 原生电流模式下，将关节力矩换算为电流 | [模式与 CAN 分组](docs/MOTOR_PROTOCOL.md#gm6020明确区分两种模式) · [采集与辨识](docs/GM6020_IDENTIFICATION.md) |
| **DM4310（4310）** | MIT 纯力矩模式，`kp=kd=0` | [量程、ID 与反馈](docs/MOTOR_PROTOCOL.md#dm4310mit-纯力矩) · [采集与辨识](docs/DM4310_IDENTIFICATION.md) |

**GM6020 电流组为 `0x1FE/0x2FE`，旧电压组为 `0x1FF/0x2FF`。** 旧官方 `CAN_cmd_gimbal()` 不能原样接收电流指令；本库对旧电压模式仅提供原始组帧。详细约定见 [电机协议](docs/MOTOR_PROTOCOL.md)。

## Yaw、Pitch 和多轴怎么配置

电机型号决定报文与力矩换算；轴的结构决定角度处理、重力与行程。两种选择分别配置，一台云台可以按每根轴所用电机阅读对应指南。
...
Pitch 的详细步骤见 [Pitch 接入与整定](docs/PITCH_TUNING.md)，折叠结构与多轴协调的适用边界见 [多轴扩展](docs/MULTI_AXIS.md)。

## 参数从哪里来

已有标定参数时可直接配置控制器。需要辨识惯量、阻尼、摩擦和 Pitch 重力时，按 **[6020](docs/GM6020_IDENTIFICATION.md) / [4310](docs/DM4310_IDENTIFICATION.md) 指南采集与标定 → [离线工具](docs/IDENTIFICATION.md)独立验证 → 将候选参数接入控制器** 的顺序进行；[在线 RLS（递推最小二乘）](docs/ONLINE_IDENTIFICATION.md)按需加入。

[测试数据清单与模板](docs/TEST_DATA.md)以已核对的双 GM6020 旧工程为例，逐项标出已有数据和需要新增的记录点。DM4310 的反馈字段按其专用指南采集；两种电机共用后续 CSV 格式与辨识工具。
...
提交 [Issue](https://github.com/LYHrmer/gimbal-smc-controller/issues) 时，请附电机型号/固件/模式、MCU、控制周期、轴与单位约定、参数，以及实际采集到的目标/反馈、命令和状态；字段来源见[测试数据清单](docs/TEST_DATA.md)。
